...
class UR_Robot:
    def __init__(self, tcp_host_ip="192.168.56.101", tcp_port=30003, workspace_limits=None,
        ...
        self.cam_intrinsics = np.array([[614.3197631835938, 0.0, 321.4326477050781, 0.0, 613.1903686523438, 245.6233367919922, 0.0, 0.0, 1.0]]).reshape(3,3)
    ...
    def testRobot(self):
        try:
            ...
            self.move_j_p([0.32, 0.25, 0.15, -2.98, -1.02, 0.67])
            # self.move_j([-np.pi, -np.pi/2, np.pi/2, 0, np.pi/2, np.pi], 0.8, 0.8)
            # self.move_j([-(140 / 360.0) * 2 * np.pi, -(120 / 360.0) * 2 * np.pi,
            #               (90 / 360.0) * 2 * np.pi, -(180 / 360.0) * 2 * np.pi,
            #               -(90 / 360.0) * 2 * np.pi, (270 / 360.0) * 2 * np.pi])
            # self.go_home()
            # g_home = [0.4,0,0.4,-np.pi,0,0]
            # self.move_j_p(g_home, 0.8, 0.8)
            # o_size = 1.40
            # open_pos = int(-258*o_size +230)  # open size:0~0.85cm --> open pos:230~10
            # self.gripper.move_and_wait_for_pos(open_pos, 255, 125)
            # print("gripper open size:")
            # self.log_gripper_info()

            # self.move_j([-(0 / 360.0) * 2 * np.pi, -(90 / 360.0) * 2 * np.pi,                    # 初始 零位
            #                  (0 / 360.0) * 2 * np.pi, -(90 / 360.0) * 2 * np.pi,
            #                  -(0 / 360.0) * 2 * np.pi, 0.0])
            ...
            #                  (0 / 360.0) * 2 * np.pi, -(90 / 360.0) * 2 * np.pi,
            #                  -(0 / 360.0) * 2 * np.pi, 0.0])
            # self.move_j_p([0.3,0,-0.1,np.pi/2,0,0],0.5,0.5)
            # for i in range(10):
            #     self.move_j_p([0.3, 0, -0.1, np.pi, 0, i*0.1], 0.5, 0.5)
            #     time.sleep(1)
    ...
    def plane_grasp(self, position, yaw=0, open_size=0.65, k_acc=0.8,k_vel=0.8,speed=255, force=125, rpy = None):
        if rpy == None:
